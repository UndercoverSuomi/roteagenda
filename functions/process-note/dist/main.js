...
function normalizeSuggestion(value, rawNoteId, createdAt, id) {
  ...
  const kind = value.kind === "event" ? "event" : value.kind === "project" ? "project" : "task";
...
function buildEnhancePrompt(content, projects, openTasks, existingTags, otherNotes, today, locale) {
  ...
  return [
    ...
    "- projectId: die ID des am besten passenden aktivierten Projekts, sonst null. Projekte sind nicht nur Aufgaben-Container, sondern das Kategorisierungssystem der App \u2014 auch f\xFCr Ideen, Links, Fotos und Wissens-Notizen. Ordne gro\xDFz\xFCgig zu, sobald ein echter thematischer Bezug besteht.",
...
var main_default = async ({ req, res, log, error }) => {
  ...
  try {
    ...
    const suggestionWarning = failedSuggestions === 0 ? null : locale === "en" ? `${failedSuggestions} of ${suggestions.length} suggestions could not be saved.` : `${failedSuggestions} von ${suggestions.length} Vorschl\xE4gen konnten nicht gespeichert werden.`;
    await databases.updateDocument(DATABASE_ID, NOTES_ID, noteId, {
      content: content.slice(0, 8e3),
    ...
    log(`Fertig: ${suggestions.length - failedSuggestions}/${suggestions.length} Vorschl\xE4ge`);
