...
- Notiz anlegen — im Notizen-Grid, per Schnellnotiz, per Mikrofon eingesprochen (Transkription ueber ein audiofaehiges OpenRouter-Modell, Standard `xiaomi/mimo-v2.5`, Override via `OPENROUTER_TRANSCRIBE_MODEL`) oder als Foto eines Notizzettels bzw. Screenshot (OCR ueber ein bildfaehiges Modell, Standard `google/gemini-3.1-flash-lite`, Override via `OPENROUTER_VISION_MODEL`)
- Links und Screenshots einwerfen (Notizen-Screen oder Schnellnotiz): Die Notiz erscheint sofort mit dem Status "wird analysiert" — ein asynchroner Notiz-Worker (Appwrite Function, bis zu 300 s statt der 30-s-Grenze der Site) laedt Artikel serverseitig (mit SSRF-Schutz) und fasst sie zusammen, sieht sich YouTube-Videos wirklich an (Bild + Ton, Standard `google/gemini-3.1-flash-lite`, Override via `OPENROUTER_VIDEO_MODEL`) bzw. liest Foto-Uploads per OCR, und fuellt die Notiz samt Veredelung und Vorschlaegen live per Realtime. Hochgeladene Fotos bleiben dauerhaft an der Notiz angehaengt und lassen sich per Klick in der Grossansicht oeffnen
...
- Bestandsnotizen nachtraeglich einordnen: Ein Klick im Notizen-Screen schickt alle unzugeordneten Notizen durch den Worker — Zuordnungen zu vorhandenen Projekten passieren direkt (live per Realtime sichtbar), zusammengehoerige Rest-Notizen werden als gebuendelte Neues-Projekt-Vorschlaege in die Inbox gelegt (ein Klick legt das Projekt an und ordnet alle gebuendelten Notizen zu)
